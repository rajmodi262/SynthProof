...
| **Third dataset, off the census** | ⚠️ **AIM's structural advantage is Adult-specific**, and the utility ordering — the part that had replicated — **stops reproducing** on a heavily imbalanced target, where all three mechanisms tie. What does transfer: pairwise beats independent on structure on all three datasets. **Reduced grid** | [`bank/BANK_MARKETING.md`](bank/BANK_MARKETING.md) · [`bank/h1_all_families.json`](bank/h1_all_families.json) |
