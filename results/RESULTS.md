...
| **Adversary comparison** | ⭐ A second, algorithm-aware adversary. The current nearest-neighbour attack is **at chance (0.498–0.538 AUC) on every structured mechanism**; the new one reaches 0.59 and wins by up to +0.092. No published epsilon changes — the ceiling is information-theoretic and a better attack cannot lift it | [`ADVERSARY_COMPARISON.md`](ADVERSARY_COMPARISON.md) · [`adversary_comparison.json`](adversary_comparison.json) |
...
| **Cross-dataset comparison** | ⚠️ Structure ordering **disagrees** between datasets; utility ordering reproduces on the two census tables | [`acs/CROSS_DATASET.md`](acs/CROSS_DATASET.md) |
| **Third dataset, off the census** | ⚠️ **AIM's structural advantage is Adult-specific**, and the utility ordering — the part that had replicated — **stops reproducing** on a heavily imbalanced target, where no mechanism's utility advantage is detected. On structure at ε=8, no mechanism separates from independent on Bank Marketing (all intervals overlap). **Full preregistered 5 × 5 grid** (75 cells, replaces initial reduced run) | [`bank/BANK_MARKETING.md`](bank/BANK_MARKETING.md) · [`bank/h1_all_families.json`](bank/h1_all_families.json) |
