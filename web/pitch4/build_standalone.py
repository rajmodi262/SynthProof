...
Run from the repository root:
    ...
    python web/pitch4/build_standalone.py

Writes `SynthProof-Pitch-Final.html` beside the SynthProof folder. The normal served build in
...
OUT = REPO.parent / "SynthProof-Pitch-Final.html"
DIST = REPO / "synthproof" / "api" / "pitch4"
...
def run_build() -> None:
    ...
    proc = subprocess.run(
        [npx, "vite", "build", "--config", "pitch4/vite.config.ts"],
        cwd=WEB,
        env=env,
        capture_output=True,
        text=True,
...
def inline() -> str:
    ...
    for probe in (".slide", ".dtable", ".arch", "@font-face", "data:font/woff2"):
    ...
    banner = (
        ...
        "  Rebuild:  python web/pitch4/build_standalone.py\n"
        "-->\n"
    )
    return banner + html
