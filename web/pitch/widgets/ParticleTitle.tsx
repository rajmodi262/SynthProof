...
export function ParticleTitle() {
  ...
  useEffect(() => {
    ...
    const paint = () => {
      ...
      const c = clarityRef.current
      for (const p of ptsRef.current) {
        // Violet where the data is sharp, coral where it is noised: the deck's two colours,
        // introduced before anyone has been told what they mean.
        const mix = c * 0.75 + p.hue * 0.25
        const R = Math.round(255 + (108 - 255) * mix)
        const G = Math.round(92 + (76 - 92) * mix)
        const B = Math.round(57 + (241 - 57) * mix)
        ctx.beginPath()
        ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2)
        ctx.fillStyle = `rgba(${R},${G},${B},${0.55 + c * 0.45})`
        ctx.fill()
      }
    }
