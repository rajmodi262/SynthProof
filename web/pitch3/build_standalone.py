...
Run from the repository root:
    ...
    python web/pitch3/build_standalone.py

Writes `SynthProof-Pitch-Gold.html` beside the SynthProof folder. The normal served build in
...
OUT = REPO.parent / "SynthProof-Pitch-Gold.html"
DIST = REPO / "synthproof" / "api" / "pitch3"
...
def run_build() -> None:
    ...
    proc = subprocess.run(
        [npx, "vite", "build", "--config", "pitch3/vite.config.ts"],
        cwd=WEB,
        env=env,
        capture_output=True,
        text=True,
...
def inline() -> str:
    ...
    for probe in (".field", ".stage", ".rule", "@font-face", "data:font/woff2"):
    ...
    banner = (
        ...
        "  Rebuild:  python web/pitch3/build_standalone.py\n"
        "-->\n"
    )
    return banner + html
