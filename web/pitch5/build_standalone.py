...
OUT = REPO.parent / "SynthProof-Pitch-Cinematic-Full.html"
DIST = REPO / "synthproof" / "api" / "pitch5"
...
def run_build() -> None:
    ...
    proc = subprocess.run(
        [npx, "vite", "build", "--config", "pitch5/vite.config.ts"],
        cwd=WEB,
        env=env,
        capture_output=True,
        text=True,
...
def inline() -> str:
    ...
    banner = (
        ...
        "  Rebuild:  python web/pitch5/build_standalone.py\n"
        "-->\n"
    )
    return banner + html
