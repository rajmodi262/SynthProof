"""Independent-marginal DP generator — the ablation baseline.

Measures one noisy 1-D marginal per column and samples each column independently, so the
output carries no cross-column structure whatsoever. That is the point: it isolates how much
of a mechanism's utility comes from modelling dependence, by removing dependence entirely.

This class was previously called `AIMGenerator`, which was simply false — it has no adaptive
marginal selection, no iteration, no k-way marginals and no graphical-model inference. It was
renamed once the real AIM (`generators/aim.py`, backed by private-PGM) landed. See
