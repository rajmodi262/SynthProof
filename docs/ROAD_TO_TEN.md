...
| 4.2 | A third dataset, from outside the census | 🔄 **IN PROGRESS** | `76a14ff` · UCI Bank Marketing wired in (45,211 rows, 13 cols, target 11.7% positive vs Adult's ~24%), 9 tests. `duration` excluded as target-leaking on UCI's own advice. **Two declared bounds were clipping real rows** (`previous` 0..60 vs a true tail at 275; `balance` −8000 vs −8019) and the test that should have caught it could not — it compared against data `TabularDataset` had already clipped to those same bounds. Reduced H1 (ε∈{1,8}, 3 seeds, 18 cells) running |
