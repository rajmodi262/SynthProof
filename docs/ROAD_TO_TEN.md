...
| 4.2 | A third dataset, from outside the census | ✅ **DONE** | Full 5 × 5 grid (75 cells, 1,771 s; replaces initial reduced run) · UCI Bank Marketing, non-census, target 11.7% positive. **Two declared bounds were clipping real rows** and the test that should have caught it *could not* — it compared against data already clipped to those bounds. **Result: AIM's structural advantage is Adult-specific.** On Bank, AIM (0.0489 [0.038, 0.059]) is indistinguishable from independent marginals (0.0416 [0.029, 0.052]); pairwise wins (0.0289 [0.011, 0.055]). And the **utility ordering — the one thing that HAD replicated across both census tables — stops reproducing**: all three mechanisms tie (0.454–0.482, TRTR 0.561). What transfers: pairwise beats independent on structure on all 3 datasets |
