...
| 4.2 | A third dataset, from outside the census | ✅ **DONE** | Full 5 × 5 grid (75 cells, 1,771 s; replaces initial reduced run) · UCI Bank Marketing, non-census, target 11.7% positive. **Two declared bounds were clipping real rows** and the test that should have caught it *could not* — it compared against data already clipped to those bounds. **Result: AIM's structural advantage is Adult-specific.** On Bank, AIM (0.0489 [0.0377, 0.0592]) is indistinguishable from independent marginals (0.0416 [0.0294, 0.0523]), and pairwise (0.0289 [0.0107, 0.0550]) overlaps both. And the **utility ordering — the one thing that HAD replicated across both census tables — stops reproducing**: no mechanism utility advantage is detected (0.454–0.482, TRTR 0.561, overlapping intervals). Pairwise does NOT separate from independent on Bank at ε=8. |
...
| 5.4 | Merge to `master`; tag `v1.0.0` | ✅ **DONE** | Annotated tag `v1.0.0` on `28dc59d` (tag object `9999417`), pushed. `master` **fast-forwarded** `256c67f..28dc59d` — verified an ancestor first, no merge commit. Tagged only after CI run `34759699144` was green on that exact commit (3.11/3.12/3.13 + console, security, auditor-guard, calibration-guard). Versions had been stated four ways (0.1.0 ×2, 0.2.0 ×2, plus a hardcoded 0.2.0 in the API); now 1.0.0 everywhere, the API derived from `__version__`. `CHANGELOG.md` leads with known gaps. **No GitHub Release was created, deliberately:** Zenodo archives only Releases published after its webhook is on (P5.5) |
| 5.5 | Mint a Zenodo DOI; verify `docker-compose up` on a clean machine | ⚠️ **PARTIAL — DOI done; the Docker runtime check is still UNRUN** | **DOI minted 2026-09-14** from the GitHub Release `v1.1.0`: concept DOI `10.5281/zenodo.22746910` (all versions — cite this) and version DOI `10.5281/zenodo.22746911` (v1.1.0 exactly). v1.0.0 was never archived, because no Release was created from it before the webhook was enabled. Docker, below, is unchanged. · **Docker Desktop's daemon is not running on this machine** (CLI present, `docker info` exits 1), so `docker compose up` was **not executed** and is not claimed. Static review found four real defects and fixed three: (1) the Dockerfile said *multi-stage* and had **one stage**, shipping `build-essential`; (2) **it never built the web console** — the console is a gitignored build artefact, so on a clean clone the image served an API with **no UI**, breaking Gate 5's own promise; (3) **no `.dockerignore` at all**, so every build uploaded `.venv311/`, `node_modules/`, `data/` and `results/`. Now three genuine stages, non-root uid 10001, console built from source in-image. (4) **AIM is unavailable in the container** — `mbi` installs from git and is not a dependency, the same gap that hid real AIM from CI until 2026-09-12; documented in the Dockerfile rather than silently tolerated. `.zenodo.json` prepared and validated; **minting the DOI needs the owner's own Zenodo account and is not automatable** |
...
| Date | Change |
|---|---|
| 2026-09-14 | **Zenodo DOI minted** from GitHub Release `v1.1.0`. Concept DOI `10.5281/zenodo.22746910`, version DOI `10.5281/zenodo.22746911`. The first wiring put the *version* DOI in the README badge and `CITATION.cff` and landed only on `release/v1.1.0`, not `master`; corrected so the badge and `doi:` use the concept DOI and `identifiers:` carries both, as `ARTIFACT.md` §7 specifies. |
| 2026-09-14 | **Full-grid Bank Marketing conclusions corrected (F2 / R3 / R4).** Generated tables via `compare_bank_full.py --write` (N1) show pairwise correlation error interval at ε = 8 widened from [0.006, 0.029] in the reduced run to [0.0107, 0.0550], overlapping independent (0.0416 [0.0294, 0.0523]). The reduced-run separation of pairwise vs independent on structure did not survive the full grid; no mechanism separates on structure or utility on Bank Marketing at ε = 8. |
...
| 2026-09-14 | **Bank Marketing full 5 × 5 preregistered grid complete (75 cells, 1,771 s).** Replaces the initial reduced run. Utility: pairwise 0.4824 [0.4501, 0.5137], independent 0.4792 [0.4708, 0.4876], AIM 0.4536 [0.3963, 0.4926] (all overlapping, below TRTR 0.5610). On structure at ε = 8, pairwise is 0.0289 [0.0107, 0.0550] (overlapping independent 0.0416 [0.0294, 0.0523]), and AIM is 0.0489 [0.0377, 0.0592] (also overlapping). No mechanism separated on structure at ε = 8. |
