...
5. **Empirical Multi-Dataset Evaluation with Transparent Retractions** (*evaluated*): We conduct a comprehensive empirical evaluation of tabular mechanism families (H1 across UCI Adult, ACSIncome, and UCI Bank Marketing), demographic subgroup leakage parity (H2), and weighted privacy allocation (H3). We document the evolution of our findings, demonstrating why null results under Benjamini-Hochberg false discovery rate control and two-one-sided-test (TOST) equivalence testing must be reported with full methodological fidelity.
...
- **Stage 0 (Pre-Flight Refusal Gate)**: The pipeline inspects public metadata (a declared or DP-counted release size $n$ and attribute schema $\mathcal{S}$; never the table's exact row count, which is private under add/remove-one) without accessing data records. Requests with extreme sparsity, insufficient samples ($n < 50$), or explosive state space ($|\mathcal{X}| > 10^9$) are rejected immediately with structured diagnostic recommendations.
...
7. **Dataset Specificity of Mechanism Advantage**: As shown in Chapter 7 and `results/bank/BANK_MARKETING.md`, the structural superiority of AIM observed on UCI Adult does not transfer across domains. On ACSIncome at $\varepsilon = 8$, pairwise achieved lower correlation error than AIM ($0.0202 \text{ [0.0076, 0.0383]}$ vs $0.0626 \text{ [0.0432, 0.0753]}$, non-overlapping), while on UCI Bank Marketing at $\varepsilon = 8$ no mechanism separated on structure (all intervals overlapped).
