...
[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.22746910.svg)](https://doi.org/10.5281/zenodo.22746910)
...
operation that touches the sensitive table to a privacy accountant, records the spend in a
hash-chained signed ledger whose head commits to `(entry_count, tip_hash)`, and audits the release against empirical attacks — reporting both
...
| **Hash-chained signed ledger** | ✅ Working | Ed25519 over a SHA-256 hash chain **plus a signed head** committing to `(entry_count, tip_hash)`. Stops modification, insertion, reordering, replay and **truncation** — **nine distinct attacks** covered by 14 tests in [tests/test_ledger_adversarial.py](tests/test_ledger_adversarial.py) — each attack run against live SQLite by an adversary with file access but no key, the remaining tests pinning correct behaviour (untampered, empty, head-tracks-tip, clear/re-append). Does not stop an adversary who holds the signing key. |
...
| **Pre-flight refusal** | ✅ Working | Refuses inputs that cannot be released honestly — too few rows, a near-unique identifier column, free text, no categorical target, a 2-way domain blow-up — using **only the declared schema and the public release size**, never the data. See [docs/design/USER_FACING_SYSTEM.md](docs/design/USER_FACING_SYSTEM.md) §2.5. |
...
| **Canary auditor** | ✅ Working, and its limits are measured | Paired Clopper-Pearson *and* the one-run Steinke construction. The **detection floor is now measured** ([results/DETECTION_FLOOR.md](results/DETECTION_FLOOR.md)): at 400 canaries the auditor resolves a 25% leak; at 10 canaries it needs a 100% leak. The **audit ceiling** `log(r/ln(1/α))` is reported beside every ε_audited, so a 0 is never mistaken for evidence of no leakage. Reporting a measurement's operating range inside the artefact is a transfer of **limit-of-detection reporting** from analytical chemistry — MIQE 2.0 (Bustin et al., *Clinical Chemistry* 2025;71(6):634–651) mandates LoD disclosure, and a lab reports *"Not Detected, < LOD"* rather than zero. The convention is not ours; the transfer is. |
